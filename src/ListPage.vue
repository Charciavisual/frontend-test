...
<template>
  <div class="contentlist">
    <div class="contentlist-header">
      ...
      <div class="search">
        <input type="text" placeholder="Search..." :value="pageParams.keyword" @input="updateKeyword($event.target.value)">
      </div>
      <!-- <div class="dp-filters">
        <span v-for="item in pageParams.filter" :key="'selected-filter_'+item">{{transCateNoToName(item)}}</span>
      </div> -->
     ...
     <div class="contentlist-body">
        <ul class="contents">
          <li v-for="(item,index) in replacedContents" :key="'contents_'+item.no" :data-no="item.no">
            <content-main :category="transCateNoToName(item.category_no)" :content="item" @click="goDetailPage(item.no)"></content-main>  
...
export default {
    ...
    replacedContents() { 
      if(this.pageParams.keyword !== "") {
        ...
        this.contents.forEach(item => {
          if(item.title.toLowerCase().includes(this.pageParams.keyword.toLowerCase()) 
          || item.contents.toLowerCase().includes(this.pageParams.keyword.toLowerCase())){
            retList.push(item);
          }
        });
        return retList; 
  ...
  methods: {
    ...
    initContents() {
      this.getAds().then(_ads => { //광고목록 초기화
        this.ads = _ads;
      })
      .then(() => { //디테일 페이지로 이동하기 전 저장한 페이지 정보가 있다면 불러온다.
        ...
        else {  //없다면 새로 리스트를 불러온다.
          this.getContents().then(_contents => {
            this.contents = _contents;
          });
        }
      }).then((scroll)=>{ //페이지 이동 전 저장한 스크롤 위치로 복귀
